...
export default {
  ...
  methods: {
    ...
    async _genResult(data) {
      ...
      if (data.song) {
        ret = ret.concat(await this._normalizeSongs(data.song.list))
      }
    ...
    async _normalizeSongs(list) {
      ...
      list.forEach(musicData => {
        req.push(getSongVkey(musicData.songmid))
      })
      const urlRes = await Promise.all(req)
      list.forEach((musicData, index) => {
        const url = parseSongUrl(urlRes[index])

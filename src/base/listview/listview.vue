...
export default {
  ...
  methods: {
    ...
    onShortcutTouchMove(e) {
      ...
      const delta = (this.touch.y2 - this.touch.y1) / ANCHOR_HEIGHT | 0 // | 0 相当Math.floor
    ...
    _scrollTo(index) {
      if (!index && index !== 0) { // index === 0可以往下走; 处理touchStart的特殊情况[invalid area]
        return
      }
      if (index < 0) { // 处理touchMove的特殊情况[invalid area]
        index = 0
  ...
  watch: {
    ...
    scrollY(newY) { // newY 滑动组件在滑动区域的纵轴偏移
